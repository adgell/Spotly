    ...
    <div style={{
      ...
      {src ? (
        ...
        <div style={{
          ...
          color: '#a8a29e', fontSize: '11px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase',
          background: '#f5f5f4', fontFamily: 'Inter, system-ui, sans-serif',
        }}>
      ...
      {images.length > 1 && (
        <>
          ...
          <div style={{
            ...
            {images.map((_, i) => (
              <div key={i} style={{
                width: i === idx ? '12px' : '4px', height: '4px', borderRadius: '2px',
                background: i === idx ? 'white' : 'rgba(255,255,255,0.4)', transition: 'width 0.2s',
              }} />
            ))}
          </div>
        </>
...
function navBtnStyle(side: 'left' | 'right'): React.CSSProperties {
  return {
    position: 'absolute', top: '50%', transform: 'translateY(-50%)', [side]: '8px', zIndex: 4,
    width: '26px', height: '26px', borderRadius: '50%',
    background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(4px)', border: 'none',
    color: '#1c1917', fontSize: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center',
    cursor: 'pointer', lineHeight: 1, boxShadow: '0 2px 6px rgba(0,0,0,0.08)',
  };
}
...
export function SpotCard({ spot, isSaved, onToggleSave }: SpotCardProps) {
  ...
  return (
    <div style={{
      ...
      fontFamily: 'Inter, system-ui, -apple-system, sans-serif', transition: 'all 0.2s ease',
    }}
      onMouseEnter={e => {
        (e.currentTarget as HTMLDivElement).style.borderColor = '#1c1917';
        (e.currentTarget as HTMLDivElement).style.boxShadow = '0 12px 28px rgba(0,0,0,0.06)';
      }}
      onMouseLeave={e => {
      ...
      <div style={{ position: 'relative' }}>
        ...
        <button
          ...
          style={{
            ...
            background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(8px)', border: 'none',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.06)',
          }}
        >
          {isSaved
            ? <BookmarkCheck style={{ width: '14px', height: '14px', color: '#ef4444' }} />
            : <Bookmark style={{ width: '14px', height: '14px', color: '#1c1917' }} />}
        </button>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              {spot.chineseName && (
                <p style={{
                  fontSize: '12px', color: '#78716c', marginTop: '3px', margin: 0,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            {cat && (
              <span style={{
                ...
                background: '#1c1917', padding: '3px 8px', borderRadius: '4px',
                letterSpacing: '0.05em', textTransform: 'uppercase'
              }}>{cat}</span>
            )}
            {spot.neighborhood && (
              <span style={{ fontSize: '12px', color: '#44403c', fontWeight: 500 }}>
                {spot.neighborhood}
              </span>
            )}
          </div>
          ...
          {spot.vibes && spot.vibes.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {spot.vibes.slice(0, 3).map(tag => (
                <span key={tag} style={{
                  fontSize: '11px', fontWeight: 500, color: '#1c1917',
                  background: '#f5f5f4', padding: '2px 8px', borderRadius: '4px'
                }}>
                  #{tag.toLowerCase()}
                </span>
              ))}
          ...
          {/* Luxury Editorial Styled Local Tip container */}
          {spot.localTip && (
            <div style={{
              background: '#fdfbfc',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px', color: '#ef4444' }}>
                <Sparkles style={{ width: '11px', height: '11px', fill: 'currentColor' }} />
                <span style={{
        ...
        <div style={{ display: 'flex', gap: '6px', width: '100%' }}>
          ...
          {spot.amapUrl && (
            <a href={spot.amapUrl} target="_blank" rel="noopener noreferrer"
              ...
              <span style={{ fontWeight: 600 }}>Navigate</span>
            </a>
          )}
        </div>
      </div>
